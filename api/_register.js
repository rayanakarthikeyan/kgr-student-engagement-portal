...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (cleanText(body.name).length < 2 || cleanText(body.name).length > 120)
      return res
        .status(400)
        .json({ error: "Enter your full name (2-120 characters)" });
    ...
    if (!["CSE", "CSM", "CSD"].includes(department))
      return res.status(400).json({ error: "Choose CSE, CSM or CSD" });
    if (!["1", "2", "3", "4"].includes(year))
      return res.status(400).json({ error: "Choose a year between 1 and 4" });
    if (!["A", "B", "C", "D", "E"].includes(section))
      return res.status(400).json({ error: "Choose a section from A to E" });
    if (!/^\S+@\S+\.\S+$/.test(email))
      return res
        .status(400)
        .json({ error: "Enter a valid institutional email address" });
    if (password.length < 8)
      return res
        .status(400)
        .json({ error: "Password must contain at least 8 characters" });
    if (rollNumber.length < 4 || rollNumber.length > 32)
    ...
    const [
      ...
      { data: rollMatches, error: rollError },
    ] = await Promise.all([
    ...
    const { data, error } = await supabase
      ...
      .insert(payload)
      .select(
        "id,name,email,role,title,roll_number,batch,contact_number,department,year,section,college,is_active,created_at",
      )
      .single();
    if (error) throw error;
    return res
      .status(201)
      .json({ user: safeUser(data), token: createSessionToken(data) });
  } catch (error) {
    if (error.code === "23505")
      return res
        .status(409)
        .json({
          error: "An account already exists for this email or roll number",
        });
