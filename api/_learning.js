...
function builtInTutor(message, assignmentTitle, questionTitles) {
  ...
  const context =
    questionTitles.length > 0
      ? ` Your assignment focuses on ${questionTitles.join(", ")}.`
      : "";
  ...
  if (
    ...
    const sqlHint =
      value.includes("having") || value.includes("group")
...
async function refreshAssignmentCounts(supabase, assignmentId) {
  ...
  const reviewed = (submissions || []).filter((record) =>
    ["graded", "reviewed"].includes(record.status),
  ).length;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (query.kind === "submission" && actor.role === "student")
        request = request.eq("author_id", actor.id);
      if (query.authorId && actor.role !== "student")
        request = request.eq("author_id", cleanText(query.authorId));
      if (query.kind) request = request.eq("kind", cleanText(query.kind));
      if (query.assignmentId)
        request = request.eq("assignment_id", cleanText(query.assignmentId));
      const [{ data, error }, { data: people, error: peopleError }] =
        await Promise.all([
          request,
          actor.role === "student"
            ? supabase
    ...
    if (req.method === "POST" && action) {
      ...
      if (action === "submit_started") {
        ...
        return res
          .status(200)
          .json({ records: data || [], count: data?.length || 0 });
      }
      const record = await findRecord(supabase, cleanText(body.id));
      if (!record || record.kind !== "submission")
      ...
      if (action === "scan") {
        ...
        const comparisons = (submissions || [])
          ...
          .map((item) => ({
            id: item.id,
            score: similarity(record.body, item.body),
          }));
        const closest = comparisons.toSorted(
          (left, right) => right.score - left.score,
        )[0] || { id: null, score: 0 };
        const nextMetadata = {
          ...metadata(record.metadata),
          similarity_score: closest.score,
          similarity_match_id: closest.id,
          similarity_note:
            closest.score >= 60
              ? "Review recommended"
              : "No strong text match found",
        };
        const { data, error: updateError } = await supabase
          .from("learning_records")
          .update({
      ...
      if (action === "grade") {
        ...
        for (const question of questions || []) {
          ...
          const keywords = Array.isArray(question.metadata?.keywords)
          ...
          possible += maxMarks;
          earned +=
            keywords.length > 0 ? maxMarks * (matched / keywords.length) : 0;
        }
        const score = possible > 0 ? Math.round(earned * 10) / 10 : 0;
        const nextMetadata = {
          ...metadata(record.metadata),
          auto_graded: true,
          auto_grade_possible: possible,
          auto_grade_summary:
            possible > 0
              ? `Matched rubric keywords across ${questions.length} question${questions.length === 1 ? "" : "s"}. Faculty review is required.`
              : "No linked rubric questions were found.",
        };
        const { data, error: updateError } = await supabase
          .from("learning_records")
          .update({
    ...
    if (req.method === "POST") {
      ...
      if (!kinds.has(kind))
        return res.status(400).json({ error: "Invalid learning record type" });
      if (kind === "question" && !["faculty", "admin"].includes(actor.role))
        return res
          .status(403)
          .json({ error: "Only faculty can add questions" });
      if (["submission", "chat"].includes(kind) && actor.role !== "student")
        return res
          .status(403)
          .json({ error: "Only students can create this record" });
      ...
      if (kind === "chat") {
        ...
        const assignmentId =
          cleanText(body.assignmentId || body.assignment_id) || null;
        ...
        const reply = {
          answer: builtInTutor(
            message,
            assignments?.[0]?.title || "this assignment",
        ...
        const payload = {
          ...
          metadata: {
            ...
            model: reply.model,
            assignment_title:
              assignments?.[0]?.title || "General assignment support",
          },
          updated_at: new Date().toISOString(),
        };
        const { data, error } = await supabase
      ...
      if (kind === "submission") {
        if (
          ...
          return res
            .status(400)
            .json({ error: "Students can only save drafts or submit work" });
        if (!assignmentId)
          return res.status(400).json({ error: "Assignment id is required" });
        if (existing && existing.author_id !== actor.id)
          return res
            .status(403)
            .json({ error: "You cannot modify this submission" });
        if (existing && existing.status !== "draft")
          return res
            .status(409)
            .json({ error: "A submitted response cannot be changed" });
        ...
        const deadline = new Date(
          `${submissionAssignment.due_date}T18:29:59.999Z`,
        );
        if (
      ...
      let serverScore =
        ...
        cleanText(body.status) !== "draft"
          ? (Array.isArray(submissionAssignment.questions)
              ? submissionAssignment.questions
              : []
            ).reduce((sum, question) => {
              const answer = submittedMetadata.answers?.[question.id];
              return (
                sum +
                (Number.isInteger(answer) && answer === question.correctIndex
      ...
      let finalStatus =
        ...
        existing?.status ||
        (kind === "submission" ? "submitted" : "active");
...
Respond with ONLY a valid JSON object matching this schema:
{
  ...
  "feedback": "string" // Constructive feedback for the student explaining the grade
          ...
          serverScore = Math.max(
            0,
            Math.min(maxMarks, Number(evaluation.score) || 0),
          );
          submittedMetadata.faculty_feedback =
            evaluation.feedback || "Graded automatically by AI.";
      ...
      const payload = {
        ...
        subject_id:
          submissionAssignment?.subject_id ||
          cleanText(body.subjectId || body.subject_id) ||
          existing?.subject_id ||
          null,
      ...
      if (payload.body.length > 100000)
        return res
          .status(413)
          .json({ error: "Submission content is too large" });
      ...
      if (kind === "submission")
        await refreshAssignmentCounts(supabase, payload.assignment_id);
      return res.status(existing ? 200 : 201).json({ record: result.data });
    }

    const id = cleanText(body.id || getQuery(req).id);
    if (!id)
      return res.status(400).json({ error: "Learning record id is required" });
    const existing = await findRecord(supabase, id);
    if (!existing)
    ...
    if (req.method === "PATCH") {
      const payload = {
        title:
          body.title === undefined ? existing.title : cleanText(body.title),
        body: body.body === undefined ? existing.body : cleanText(body.body),
        assignment_id:
          body.assignmentId === undefined && body.assignment_id === undefined
            ? existing.assignment_id
            : cleanText(body.assignmentId || body.assignment_id) || null,
        subject_id:
          body.subjectId === undefined && body.subject_id === undefined
            ? existing.subject_id
            : cleanText(body.subjectId || body.subject_id) || null,
        status:
      ...
      if (existing.kind === "submission")
        await refreshAssignmentCounts(supabase, existing.assignment_id);
      return res.status(200).json({ record: data });
    }
