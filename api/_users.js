...
function normalizeUserPayload(body, { partial = false } = {}) {
  ...
  if (
    ...
    body.roll_number !== undefined
  ) {
    payload.roll_number = cleanText(body.rollNumber || body.roll_number);
  }
  if (!partial || body.batch !== undefined)
    payload.batch = cleanText(body.batch);
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (query.department)
        request = request.eq(
          "department",
          cleanText(query.department).toUpperCase(),
        );
      if (query.section)
        request = request.eq("section", cleanText(query.section).toUpperCase());
      if (query.search) {
    ...
    if (targetUsers?.[0]?.role === "admin") {
      return res
        .status(403)
        .json({ error: "The primary Super Admin account is protected" });
    }
