...
function projectRows(rows, selectColumns, tableName, db) {
  const includeSubjects =
    tableName === "assignments" && selectColumns.includes("subjects(");
  ...
  return rows.map((row) => {
    const projected = wantsAll
      ...
      : Object.fromEntries(
          baseColumns
            .filter(
              (column) => !column.includes("(") && row[column] !== undefined,
            )
    ...
    if (includeSubjects) {
      projected.subjects =
        db.subjects.find((subject) => subject.id === row.subject_id) ?? null;
    }
...
class LocalQuery {
  ...
  or(expression) {
    const clauses = expression
      .split(",")
      .map((clause) => clause.split(".ilike."))
      .filter(([column, pattern]) => column && pattern);
  ...
  async execute() {
    ...
    if (!Array.isArray(table)) {
      return {
        data: null,
        error: new Error(`Unknown table: ${this.tableName}`),
      };
    }
    ...
    if (this.sort) {
      rows = rows.toSorted((left, right) => {
        const comparison = String(left[this.sort.column] ?? "").localeCompare(
    ...
    const data = this.selectOptions.head
      ? null
      : projectRows(rows, this.selectColumns, this.tableName, this.db);
    return {
      data: this.returnSingle ? (data?.[0] ?? null) : data,
      count,
      error: null,
...
export function createSupabaseClient({ requirePrivileged = false } = {}) {
  ...
  if (requirePrivileged && !privilegedKey) {
    if (localSeedDb) return createLocalDataClient(localSeedDb);
    throw new Error(
      "Missing Supabase service role or secret key for write operations",
    );
  }

  return createClient(
    supabaseUrl,
    requirePrivileged ? privilegedKey : supabaseKey,
  );
}
...
export function assertSubjectType(type) {
  if (!subjectTypeValues.has(type)) {
    throw new Error(
      "Subject type must be Theory Only, Lab Only, or Theory + Lab",
    );
  }
}
...
export function verifyPassword(password, storedHash) {
  ...
  const [algorithm, iterationsText, salt, expected] =
    String(storedHash).split("$");
  if (algorithm !== "pbkdf2_sha512" || !iterationsText || !salt || !expected)
    return false;
...
function verifySessionToken(token) {
  ...
  try {
    ...
    return claims.exp > Math.floor(Date.now() / 1000) && claims.sub
      ? claims
      : null;
  } catch {
    return null;
...
export async function findUserByIdentifier(
  supabase,
  identifier,
  columns = "id,name,email,password,password_hash,role,title,roll_number,batch,contact_number,department,year,section,college,is_active",
) {
...
export async function requireUser(
  ...
  if (authorization.toLowerCase().startsWith("bearer ")) {
    ...
    const { data, error } = await supabase
      .from("users")
      .select(
        "id,name,email,role,title,roll_number,batch,contact_number,department,year,section,college,is_active",
      )
      .eq("id", claims.sub)
      .limit(1);
  ...
  const password = cleanText(
    getHeader(req, "x-user-password") ||
      getHeader(req, "x-admin-password") ||
      body.userPassword ||
      body.adminPassword,
  );
