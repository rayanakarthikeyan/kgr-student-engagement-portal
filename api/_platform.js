...
const bucketedActivityKinds = new Set([
  "video_progress",
  "pdf_dwell",
  "exam_autosave",
  "editor_change",
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!table)
      return res
        .status(400)
        .json({ error: "A valid platform entity is required" });
    ...
    if (req.method === "GET") {
      ...
      if (entity === "submission" && actor.role === "student")
        request = request.eq("user_id", actor.id);
      if (entity === "activity" && actor.role === "student")
        request = request.eq("user_id", actor.id);
      if (entity === "resource" && actor.role === "student")
        request = request.eq("is_published", true);
      if (query.courseId)
        request = request.eq("course_id", cleanText(query.courseId));
      if (query.resourceId)
        request = request.eq("resource_id", cleanText(query.resourceId));
    ...
    if (req.method === "POST") {
      ...
      if (entity === "enrollment") {
        if (actor.role !== "student")
          return res
            .status(403)
            .json({ error: "Only students can self-enroll" });
        ...
        const payload = {
          ...
          user_id: actor.id,
          course_id: courseId,
          tracks: Array.isArray(body.tracks) ? body.tracks : ["theory", "lab"],
          progress: 0,
          study_minutes: 0,
          status: "active",
          created_at: now,
        ...
        const payload =
          entity === "resource"
            ...
            : {
                ...
                settings: metadata(body.settings),
    ...
    if (!["faculty", "admin"].includes(actor.role))
      return res.status(403).json({ error: "Faculty access is required" });
    if (!["resource", "assessment"].includes(entity))
      return res
        .status(403)
        .json({ error: "This entity cannot be modified here" });
