...
function normalizeAssignmentPayload(body, { partial = false } = {}) {
  ...
  if (!partial || body.questions !== undefined) payload.questions = toArray(body.questions);
  if (!partial || body.hints !== undefined) payload.hints = toArray(body.hints).map(cleanText).filter(Boolean).slice(0, 10);
  if (!partial || body.executionEnvironment !== undefined) {
    payload.execution_environment = body.executionEnvironment === "external" ? "external" : "runner";
  }
  if (!partial || body.workMode !== undefined || body.work_mode !== undefined) {
    ...
    if (!workModes.has(value)) throw new Error("Work mode must be response, mcq, or ide");
  ...
  if (!partial || body.assignmentType !== undefined || body.assignment_type !== undefined) {
    const value = cleanText(body.assignmentType || body.assignment_type).toLowerCase();
    if (!assignmentTypes.has(value)) throw new Error("Assignment type must be theory, practice, assessment, or lab");
    payload.assignment_type = value;
  }
  if (!partial || body.curriculumItemId !== undefined || body.curriculum_item_id !== undefined) {
    payload.curriculum_item_id = cleanText(body.curriculumItemId || body.curriculum_item_id);
  }
  if (!partial || body.courseCode !== undefined || body.course_code !== undefined) {
    const value = cleanText(body.courseCode || body.course_code).toUpperCase();
    if (!courseCodes.has(value)) throw new Error("Course code must be JAVA or DBMS");
    payload.course_code = value;
  }
  if (!partial || body.unitNumber !== undefined || body.unit_number !== undefined) {
    payload.unit_number = Math.min(5, Math.max(1, toInteger(body.unitNumber ?? body.unit_number, 1)));
  }
  if (!partial || body.durationMinutes !== undefined || body.duration_minutes !== undefined) {
    payload.duration_minutes = Math.min(480, Math.max(1, toInteger(body.durationMinutes ?? body.duration_minutes, 30)));
  }
  ...
  Object.keys(payload).forEach((key) => {
    if (payload[key] === "" && !["description", "starter_code", "curriculum_item_id"].includes(key)) delete payload[key];
  });
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (query.templates === "1") {
        if (!["faculty", "admin"].includes(actor.role)) return res.status(403).json({ error: "Templates are available to faculty only" });
      ...
      const assignments = actor.role === "student"
        ? (data || []).filter((assignment) => {
          ...
          questions: Array.isArray(assignment.questions)
            ? assignment.questions.map(({ correctIndex: _correctIndex, ...question }) => question)
            : [],
        }))
        : data || [];
    ...
    if (req.method === "PATCH") {
      ...
      const { data: existing, error: readError } = await supabase.from("assignments").select("*").eq("id", id).single();
      if (readError) throw readError;
      if (!existing) return res.status(404).json({ error: "Assignment not found" });
      const { data: attempts, error: attemptError } = await supabase.from("learning_records").select("id").eq("assignment_id", id).limit(1);
      if (attemptError) throw attemptError;
      if (attempts?.length) return res.status(409).json({ error: "Students have started this assignment. Publish a new copy to change it." });
...
function validateAssignment(payload) {
  if (!payload.assigned_user_ids?.length) return "Select at least one registered student";
  if (!/^\d{4}-\d{2}-\d{2}$/.test(payload.due_date || "") || !Number.isFinite(Date.parse(payload.due_date))) return "A valid deadline is required";
  if (payload.assignment_type === "theory") return "Assign theory links through Theory resources";
  if (payload.assignment_type === "assessment" && payload.work_mode !== "mcq") return "Assessments currently support proctored MCQs";
  if (payload.work_mode === "mcq") {
    const qs = payload.questions;
    if (!Array.isArray(qs) || qs.length < 1 || qs.length > 100) return "Add 1-100 questions";
    if (new Set(qs.map(q => q?.id)).size !== qs.length) return "Question IDs must be unique";
    if (qs.some(q => !q || !cleanText(q.id) || !cleanText(q.prompt) || !Array.isArray(q.options) || q.options.length !== 4 || q.options.some(o => !cleanText(o)) || !Number.isInteger(q.correctIndex) || q.correctIndex < 0 || q.correctIndex > 3 || !Number.isFinite(q.marks) || q.marks <= 0)) return "Complete each question, four options, correct answer and marks";
    payload.max_marks = qs.reduce((sum, q) => sum + q.marks, 0);
  } else if (!payload.test_cases?.[0]?.output?.trim()) return "Add expected output or expected observations";
  return null;
}
