...
function pageTitle(view: ViewId) {
  const titles: Record<ViewId, [string, string]> = {
    ...
    "java-learn": [
      "OOP Java: Theory",
      "Study faculty-assigned videos and documents organized across 5 units.",
    ],
    "java-lab": [
      "OOP Java: Lab work",
      "Complete 21 official Java experiments in an instrumented coding workspace.",
    ],
    "dbms-learn": [
      "DBMS: Theory",
      "Study faculty-assigned videos and documents organized across 5 units.",
    ...
    "dbms-lab": [
      "DBMS: Lab work",
      "Complete 10 official DBMS experiments in an instrumented coding workspace.",
    ],
    coursework: [
      "Practice",
      "Complete non-proctored MCQs and IDE questions for JAVA and DBMS.",
    ],
    assessment: [
      "Assessment center",
      "Secure, timed checkpoints with autosave and integrity monitoring.",
    ],
    resources: [
...
export default function App() {
  ...
  const [theme, setTheme] = useState<Theme>(() =>
    localStorage.getItem("kgr-theme") === "dark" ? "dark" : "light",
  );
  ...
  if (checkingSession)
    return (
      <main className="grid min-h-screen place-items-center bg-[var(--page)] text-[var(--ink)]">
  ...
  return (
    <div className="min-h-screen bg-[var(--page)] text-[var(--ink)] transition-colors">
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-[268px] flex-col border-r border-[var(--line)] bg-[var(--sidebar)]/95 px-4 py-5 backdrop-blur-xl transition-transform lg:translate-x-0 ${mobileNavOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <nav className="mt-9 space-y-1" aria-label="Primary navigation">
          <p className="mb-3 px-3 text-[10px] font-bold uppercase tracking-[.16em] text-[var(--muted)]">
            Workspace
          </p>
          {navigation.map((item, i) => {
            if ("isHeader" in item) {
              return (
                <p
                  key={`header-${i}`}
                  className="mb-2 mt-4 px-3 text-[10px] font-bold uppercase tracking-[.16em] text-[var(--muted)]"
                >
                  {item.label}
                </p>
              );
        ...
        <div className="mt-auto rounded-lg border border-[var(--line)] bg-[var(--surface)] p-3">
          <div className="flex items-center gap-3">
            <span className="grid size-9 shrink-0 place-items-center rounded-lg bg-cyan-400/15 text-sm font-bold text-cyan-500">
              {session.user.name
                .split(" ")
                .map((part) => part[0])
                .join("")
      ...
      <div className="lg:pl-[268px]">
        <header className="sticky top-0 z-30 flex h-[72px] items-center border-b border-[var(--line)] bg-[var(--page)]/85 px-4 backdrop-blur-xl sm:px-7 lg:px-9">
          <button
            className="icon-button mobile-only mr-3"
          ...
          <div className="flex items-center gap-2">
            <span className="hidden items-center gap-2 rounded-lg border border-emerald-500/20 bg-emerald-500/8 px-3 py-2 text-xs font-medium text-emerald-500 md:flex">
            ...
            <button
              className="hidden items-center gap-2 rounded-lg border border-[var(--line)] bg-[var(--surface)] px-3 py-2 text-sm font-medium sm:flex"
              type="button"
            >
              {session.user.role === "student" ? (
