  ...
  return {
    ...
    courseCode: (row.course_code ||
      (row.course_id === "course-dbms"
        ? "DBMS"
        : "JAVA")) as LearningResource["courseCode"],
    unitNumber: Number(row.unit_number || 1),
    dueDate: String(row.due_date || ""),
    assignedUserIds: Array.isArray(row.assigned_user_ids)
      ? row.assigned_user_ids.map(String)
      : [],
  };
}
...
export async function loadAiChatLogs(
  token: string,
  userId: string,
): Promise<any[]> {
  const data = await parseResponse(
    await fetch(`${API_BASE}/api/ai-chat-logs?userId=${userId}`, {
      headers: authHeaders(token),
    }),
  );
...
async function platformMutation(
  ...
  return parseResponse(
    await fetch(`${API_BASE}/api/platform?entity=${entity}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
...
export async function enrollInCourse(token: string, courseId: string) {
  return platformMutation(token, "enrollment", {
    courseId,
    tracks: ["theory", "lab"],
  });
}
...
export async function deleteResource(token: string, id: string) {
  await parseResponse(
    await fetch(
      `${API_BASE}/api/platform?entity=resource&id=${encodeURIComponent(id)}`,
      {
        method: "DELETE",
        headers: authHeaders(token),
      },
...
export async function loadResourceActivity(token: string) {
  const data = await parseResponse(
    await fetch(`${API_BASE}/api/platform?entity=activity&summary=1`, {
      headers: authHeaders(token),
    }),
  );
...
export async function createAssessment(
  ...
  return platformMutation(token, "assessment", {
    ...assessment,
    status: "draft",
    questions: [],
  });
...
export async function submitAssessment(
  ...
  return platformMutation(token, "submission", {
    ...submission,
    status: submission.automatic ? "auto_submitted" : "submitted",
  });
}
...
export async function loadCoursework(token: string, includeRoster: boolean) {
  const requests = [
    ...
    fetch(
      `${API_BASE}/api/learning?kind=submission${includeRoster ? "&summary=1" : ""}`,
      { headers: authHeaders(token) },
    ),
  ] as const;
  const [
  ...
  const [assignmentData, subjectData, rosterData, learningData] =
    await Promise.all([
      parseResponse(assignmentResponse),
      parseResponse(subjectResponse),
      rosterResponse
        ? parseResponse(rosterResponse)
  ...
  return {
    ...
    students: ((rosterData.users || []) as Array<Record<string, unknown>>).map(
      mapUser,
    ),
...
export async function loadActivityTemplates(
  token: string,
): Promise<Record<string, ActivityTemplate>> {
  const data = await parseResponse(
    await fetch(`${API_BASE}/api/assignments?templates=1`, {
      headers: authHeaders(token),
    }),
  );
...
export async function loadStudentWork(
  ...
  const data = await parseResponse(
    await fetch(
      `${API_BASE}/api/learning?kind=submission&authorId=${encodeURIComponent(userId)}`,
      { headers: authHeaders(token) },
    ),
  );
...
export async function loadStudentOverview(token: string) {
  const [enrollmentResponse, resourceResponse] = await Promise.all([
    fetch(`${API_BASE}/api/platform?entity=enrollment`, {
      headers: authHeaders(token),
    }),
    fetch(`${API_BASE}/api/platform?entity=resource`, {
      headers: authHeaders(token),
    }),
  ]);
  const [enrollmentData, resourceData] = await Promise.all([
    parseResponse(enrollmentResponse),
    parseResponse(resourceResponse),
  ]);
  return {
    enrollments: (
      (enrollmentData.enrollments || []) as Array<Record<string, unknown>>
    ).map((row) => ({
      id: String(row.id),
      courseId: String(row.course_id),
    ...
    resources: (
      (resourceData.resources || []) as Array<Record<string, unknown>>
    ).map(mapResource),
  };
}
...
export async function deleteCourseworkAssignment(token: string, id: string) {
  await parseResponse(
    await fetch(`${API_BASE}/api/assignments?id=${encodeURIComponent(id)}`, {
      method: "DELETE",
      headers: authHeaders(token),
...
export async function updateCourseworkAssignment(
  token: string,
  id: string,
  assignment: Record<string, unknown>,
) {
...
export async function saveCourseworkSubmission(
  token: string,
  input: {
  ...
  const data = await parseResponse(
    await fetch(`${API_BASE}/api/learning`, {
      method: "POST",
      headers: authHeaders(token),
      body: JSON.stringify({
        kind: "submission",
...
export async function bulkAutoGrade(token: string) {
  const data = await parseResponse(
    await fetch(`${API_BASE}/api/auto-grade-bulk`, {
      method: "POST",
      headers: authHeaders(token),
    }),
  );
