...
export function useProctoring({
  ...
  const emit = useCallback((kind: ActivityLog["kind"], metadata: Record<string, unknown>) => {
  ...
  const registerViolation = useCallback((reason: string) => {
    const now = Date.now();
    if (!started || submittedRef.current || now - lastViolationAt.current < 1200) return;
  ...
  useEffect(() => {
    if (!started) return;
    const onVisibility = () => { if (document.hidden) registerViolation("Tab switch detected"); };
    const onBlur = () => registerViolation("Exam window lost focus");
    const onFullscreen = () => { if (!document.fullscreenElement) registerViolation("Fullscreen mode exited"); };
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      if (next % 60 === 0) {
          sessionStorage.setItem(storageKey, JSON.stringify({ remainingSeconds: next, violations }));
  ...
  const begin = useCallback(async () => {
    ...
    if (saved) {
      try {
        const state = JSON.parse(saved) as { remainingSeconds?: number; violations?: number };
        if (state.remainingSeconds) setRemainingSeconds(state.remainingSeconds);
        if (state.violations) setViolations(state.violations);
      } catch { /* Ignore malformed local recovery state. */ }
    }
    setStarted(true);
    emit("exam_started", { durationMinutes, fullscreenRequested: true });
    try { await document.documentElement.requestFullscreen(); } catch { setWarning("Fullscreen could not be started. Keep this window focused."); }
  ...
  return { started, begin, remainingSeconds, formattedTime, violations, warning, clearWarning: () => setWarning("") };
}
