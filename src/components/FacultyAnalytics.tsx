...
export function FacultyAnalytics({ session, compact = false }: { session: AuthSession; compact?: boolean }) {
  ...
  const summaries = useMemo(() => {
    ...
    return new Map(students.map(student => {
      const work = assignments.filter(item => item.assigned_user_ids.includes(student.id));
      const history = records.get(student.id) || [];
      const events = logs.get(student.id) || [];
      const final = history.filter(item => item.status !== "draft");
      const scored = final.filter(item => typeof item.score === "number" && Number(byAssignment.get(item.assignment_id || "")?.max_marks) > 0);
      const score = scored.length ? Math.round(scored.reduce((sum, item) => sum + Number(item.score) / Number(byAssignment.get(item.assignment_id || "")?.max_marks) * 100, 0) / scored.length) : null;
      const seconds = events.filter(item => item.kind === "video_progress" || item.kind === "pdf_dwell").reduce((sum, item) => sum + Number(item.duration_seconds || 0), 0);
      return [student.id, {
        work, history, final, score, minutes: Math.round(seconds / 60),
        runs: events.filter(item => item.kind === "code_run" && byAssignment.get(item.assignment_id || "")?.assignment_type === "lab").reduce((sum, item) => sum + Number(item.event_count || 1), 0),
        pastes: events.filter(item => item.kind === "editor_paste").reduce((sum, item) => sum + Number(item.event_count || 1), 0),
        violations: events.filter(item => item.kind === "exam_violation").reduce((sum, item) => sum + Number(item.event_count || 1), 0),
        hints: history.reduce((sum, item) => sum + (Array.isArray(item.metadata.hints_used) ? item.metadata.hints_used.length : 0), 0),
      }];
    }));
  }, [students, submissions, activities, assignments]);
  const filtered = useMemo(() => students.filter(student => matchesCohort(student, query, department, section)), [students, query, department, section]);
  const selected = filtered.find(student => student.id === selectedId) || filtered[0];
  ...
  const totals = filtered.reduce((sum, student) => {
    const row = summaries.get(student.id)!;
    return { submitted: sum.submitted + row.final.length, assigned: sum.assigned + row.work.length, minutes: sum.minutes + row.minutes };
  }, { submitted: 0, assigned: 0, minutes: 0 });

  if (loading) return <div className="empty-panel min-h-72"><Clock3 size={24} className="animate-pulse" />Loading cohort data...</div>;
  const reportTable = <table><thead><tr><th>Name / Roll</th><th>Department / Section</th><th>Email / Contact</th><th>Submitted / Assigned</th><th>Study min</th><th>Average %</th></tr></thead><tbody>{filtered.map(student => {
    const row = summaries.get(student.id)!;
    return <tr key={student.id}><td>{student.name}<br />{student.rollNumber}</td><td>{student.department || "-"} / {student.section || "-"}</td><td>{student.email}<br />{student.contactNumber || "-"}</td><td>{row.final.length} / {row.work.length}</td><td>{row.minutes}</td><td>{row.score ?? "-"}</td></tr>;
  })}</tbody></table>;
  ...
  return <div className="mx-auto max-w-[1440px] space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,0.8fr)]">
      <section className="min-w-0">
        <div className="overflow-x-auto"><table className="data-table min-w-[560px]"><thead><tr><th>Student</th><th>Dept / Section</th><th>Submitted</th><th>Average</th></tr></thead><tbody>{pageStudents.map(student => {
          const row = summaries.get(student.id)!;
          return <tr key={student.id} className={selected?.id === student.id ? "selected-row" : ""}><td><button className="text-left" onClick={() => setSelectedId(student.id)}><strong>{student.name}</strong><small>{student.rollNumber || student.email}</small></button></td><td>{student.department || "-"} / {student.section || "-"}</td><td>{row.final.length} / {row.work.length}</td><td>{row.score == null ? "-" : row.score + "%"}</td></tr>;
        })}{!filtered.length && <tr><td colSpan={4}>No students match these filters.</td></tr>}</tbody></table></div>
        <footer className="mt-3 flex items-center justify-between text-xs text-[var(--muted)]"><span>{filtered.length} students / Page {currentPage + 1} of {pageCount}</span><div className="flex gap-2"><button className="icon-button" aria-label="Previous page" title="Previous page" disabled={currentPage === 0} onClick={() => setPage(currentPage - 1)}><ArrowLeft size={16} /></button><button className="icon-button" aria-label="Next page" title="Next page" disabled={currentPage + 1 >= pageCount} onClick={() => setPage(currentPage + 1)}><ArrowRight size={16} /></button></div></footer>
      </section>
      <aside className="min-w-0 border-t border-[var(--line)] pt-5 xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0">
        {selected && insight ? <><div className="flex items-start justify-between gap-2"><div><h3 className="text-lg font-semibold">{selected.name}</h3><p className="mt-1 text-xs text-[var(--muted)]">{selected.rollNumber} / {selected.department || "-"} / {selected.section || "-"}</p></div><button className="icon-button" aria-label="Print student insights" title="Print student insights" onClick={() => setReport("student")}><Printer size={17} /></button></div>
          <p className="mt-3 break-all text-sm">{selected.email}</p><p className="mt-1 text-sm">{selected.contactNumber || "Contact not recorded"}</p><p className="mt-1 text-xs text-[var(--muted)]">{selected.college}</p>
          <dl className="mt-5 grid grid-cols-2 gap-4">{[["Study minutes", insight.minutes], ["Average score %", insight.score ?? "-"], ["Lab runs", insight.runs], ["Paste events", insight.pastes], ["Hints opened", insight.hints], ["Proctor violations", insight.violations]].map(([label, value]) => <div key={label} className="border-b border-[var(--line)] pb-3"><dt className="text-xs text-[var(--muted)]">{label}</dt><dd className="mt-1 text-xl font-semibold">{value}</dd></div>)}</dl>
          <h4 className="mt-6 text-sm font-semibold">Submitted work and drafts</h4>
    ...
    {report && createPortal(<section id="faculty-print-report" className="print-report">
      <div className="report-toolbar"><button onClick={() => window.print()}><Printer size={16} />Print / Save PDF</button><button onClick={() => setReport(null)} aria-label="Close report"><X size={18} />Close</button></div>
      <h1>KG Reddy College of Engineering and Technology</h1><h2>{report === "roster" ? "Student roster and insights" : "Individual student insights"}</h2><p>{new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })} IST / {department || "All departments"} / {section || "All sections"}</p>
      {report === "roster" ? reportTable : selected && insight && <><h3>{selected.name} / {selected.rollNumber}</h3><p>{selected.department} / {selected.section} / {selected.email} / {selected.contactNumber}</p><p>Submitted {insight.final.length} of {insight.work.length}. Average {insight.score ?? "-"}%. Theory {insight.minutes} min. Lab runs {insight.runs}. Paste events {insight.pastes}. Hints opened {insight.hints}. Proctor violations {insight.violations}.</p><table><thead><tr><th>Work</th><th>Type</th><th>Deadline</th><th>Status / Marks</th></tr></thead><tbody>{insight.work.map(work => { const record = insight.history.find(item => item.assignment_id === work.id); return <tr key={work.id}><td>{work.title}</td><td>{work.assignment_type}</td><td>{work.due_date}</td><td>{record?.status || "Not started"} / {record?.score ?? "-"} of {work.max_marks}</td></tr>; })}</tbody></table></>}
