...
export function AuthScreen({
  ...
  return (
    <main className="auth-page min-h-screen overflow-hidden bg-[var(--page)] text-[var(--ink)]">
      <button
        className="icon-button fixed right-5 top-5 z-30"
      ...
      <div className="mx-auto grid min-h-screen max-w-[1480px] lg:grid-cols-[1.08fr_.92fr]">
        <section className="auth-showcase relative hidden min-h-screen flex-col justify-between border-r border-[var(--line)] p-12 lg:flex xl:p-16">
          ...
          <div className="relative z-10 flex items-center gap-3">
            <span className="grid size-11 place-items-center rounded-lg bg-cyan-400 text-slate-950">
          ...
          <div className="relative z-10 max-w-[620px]">
            ...
            >
              <span className="auth-kicker mb-5 inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold">
          ...
          <p className="relative z-10 text-xs text-[var(--muted)]">
            Built for accountable, accessible academic learning.
          </p>
        </section>
        ...
        <section className="flex min-h-screen items-center justify-center p-5 sm:p-8 lg:p-12">
          ...
          >
            <div className="mb-9 flex items-center gap-3 lg:hidden">
              <span className="grid size-10 place-items-center rounded-lg bg-cyan-400 text-slate-950">
                <Command size={21} />
              </span>
              <div>
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--muted)]">
              {registering
                ? "Use your institutional details to begin enrollment."
            ...
            <form className="mt-8 space-y-5" onSubmit={submit}>
              ...
              {registering && (
                <p className="flex items-center gap-2 text-xs text-[var(--muted)]">
                  ...
                  By continuing, you agree to the academic integrity policy.
                </p>
              )}
              <button
                className="flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-cyan-400 px-4 text-sm font-bold text-slate-950 transition hover:bg-cyan-300 disabled:opacity-60"
                disabled={loading}
                type="submit"
              >
                {loading
            ...
            <p className="mt-7 text-center text-sm text-[var(--muted)]">
              {registering ? "Already registered?" : "New student?"}{" "}
              <button
