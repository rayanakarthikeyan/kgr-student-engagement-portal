...
import {
  ...
  deleteCourseworkAssignment,
  loadActivityTemplates,
  updateCourseworkAssignment,
  loadCoursework,
  runCode,
  saveCourseworkSubmission,
} from "../platform/api";
...
function assignmentType(assignment: AssignmentRecord): CourseworkType {
  return (
    ...
    (assignment.starter_code ? "practice" : "theory")
  );
}
function courseForSubject(
  subject: AssignmentSubject | null | undefined,
): CourseCode {
...
function AssignmentWorkspace({
  ...
  const [answers, setAnswers] = useState<Record<string, number>>(() => {
    if (
      ...
      return savedAnswers as Record<string, number>;
    try {
  ...
  const [output, setOutput] = useState({
    ...
    stdout: "Run your work to see the actual output.",
    stderr: "",
    durationMs: 0,
  });
    ...
    try {
      const record = await saveCourseworkSubmission(session.token, {
        ...
        metadata: {
          ...
          output: isCoding
            ? (output.stdout + output.stderr).slice(0, 8000)
            : null,
        },
      });
      onSaved(record);
      if (status === "submitted") {
        ...
        if (isCoding) telemetry.recordSubmit();
      ...
      setError(
        caught instanceof Error ? caught.message : "Work could not be saved",
      );
    } finally {
      setSaving(false);
    }
  };
  submitRef.current = (automatic) => {
  ...
  const execute = async () => {
    ...
    try {
      ...
      const result = {
        ...
        stdout: "",
        stderr: caught instanceof Error ? caught.message : "Runner unavailable",
        durationMs: 0,
      };
      setOutput(result);
      telemetry.recordRun(result);
    ...
    return (
      <div className="mx-auto max-w-3xl py-8">
        <section className="panel p-7 sm:p-9">
          <button
            className="secondary-button mb-7"
            onClick={onBack}
            type="button"
          >
            <ArrowLeft size={16} />
          ...
          <h2 className="mt-4 text-2xl font-semibold">{assignment.title}</h2>
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-3">
            <div className="exam-stat">
              <Clock3 size={18} />
              <strong>{assignment.duration_minutes || 30} min</strong>
          ...
          <button
            ...
            disabled={isPastDeadline(assignment.due_date)}
    ...
    <div className="mx-auto max-w-[1500px] space-y-4">
      <header className="flex flex-wrap items-center gap-3 border-b border-[var(--line)] pb-4">
        ...
        <div className="min-w-0 flex-1 basis-[220px]">
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={`tag ${assignmentCourse(assignment) === "JAVA" ? "cyan" : "amber"}`}
            >
              {assignmentCourse(assignment)}
            </span>
            <span className="tag neutral">
              Unit {assignment.unit_number || 1}
            </span>
            <span className="tag neutral">{typeLabel(type)}</span>
            <span className="tag neutral">{mode.toUpperCase()}</span>
          </div>
          <h2 className="mt-2 break-words text-lg font-semibold sm:truncate">
            {assignment.title}
          </h2>
        </div>
        {type === "assessment" && proctor.started && (
          <span className="rounded-md border border-amber-500/20 bg-amber-500/10 px-3 py-2 font-mono text-sm font-semibold text-amber-600">
      ...
      {notice && (
        <div className="flex items-center gap-2 rounded-lg border border-emerald-300 bg-emerald-500/8 px-4 py-3 text-sm text-emerald-600">
          ...
          {notice}
          {locked && submission?.score != null
            ? ` Score: ${submission.score}/${assignment.max_marks}`
            : ""}
        </div>
      )}
      ...
      >
        ...
        <section className="panel min-w-0 overflow-hidden">
          <div className="flex h-11 items-center border-b border-[var(--line)] px-4">
            {isMcq ? (
              <ListChecks size={15} className="text-cyan-600" />
            ) : (
              <FileCode2 size={15} className="text-cyan-600" />
            )}
            ...
            {locked && (
              <span className="ml-auto status-pill on-track">
                Submitted
                {submission?.score != null
                  ? ` / ${submission.score}/${assignment.max_marks}`
                  : ""}
              </span>
            )}
          </div>
          {isMcq ? (
            <div className="max-h-[620px] space-y-4 overflow-y-auto bg-[var(--surface)] p-4 sm:p-5">
                ...
                >
                  ...
                  <div className="mt-3 grid gap-2">
                    {question.options.map((option, optionIndex) => (
                      <label
                        className={`flex cursor-pointer items-start gap-3 rounded-md border px-3 py-2.5 text-sm ${answers[question.id] === optionIndex ? "border-cyan-500 bg-cyan-500/8" : "border-[var(--line)]"}`}
              ...
              {assignment.questions.length === 0 && (
                <div className="empty-panel min-h-52">
                  <ListChecks size={26} />
                  <span>No questions were added to this assignment.</span>
                </div>
              )}
            </div>
          ) : isCoding ? (
            ...
            <textarea
              className="min-h-[430px] w-full resize-y bg-[var(--surface)] p-5 text-sm leading-7 text-[var(--ink)] outline-none"
              disabled={locked}
          ...
          <footer className="flex flex-wrap justify-end gap-2 border-t border-[var(--line)] p-4">
            ...
            <button
              className="secondary-button"
              disabled={saving || locked || isPastDeadline(assignment.due_date)}
...
function StudentCoursework({
  ...
  const [courseFilterState, setCourseFilterState] = useState(
    courseFilter || "",
  );
  ...
  const visible = useMemo(
    () =>
      assignments.filter(
        (item) =>
          (filter === "all" || assignmentType(item) === filter) &&
          (!courseFilterState ||
            assignmentCourse(item) === courseFilterState) &&
          (!unitFilter || String(item.unit_number) === unitFilter),
      ),
    [assignments, filter, courseFilterState, unitFilter],
  );
  const queueTitle =
  ...
  const queueDescription =
    initialType === "practice"
      ? "Non-proctored MCQs and IDE questions assigned by faculty."
      : initialType === "assessment"
    ...
    return (
      <AssignmentWorkspace
        ...
        submission={submissions.find(
          (item) => item.assignment_id === active.id,
        )}
        session={session}
        theme={theme}
        onEvent={onEvent}
  ...
  return (
    <div className="space-y-6">
      <section className="flex flex-col gap-4 border-b border-[var(--line)] pb-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[.16em] text-cyan-600">
            Assigned coursework
          </p>
          <h2 className="mt-2 text-3xl font-semibold">{queueTitle}</h2>
          <p className="mt-2 text-sm text-[var(--muted)]">{queueDescription}</p>
        </div>
      ...
      {initialType === "all" && (
        <div className="segmented-control w-full overflow-x-auto sm:w-fit">
          {(["all", "practice", "assessment", "lab"] as const).map((value) => (
            <button
      ...
      {visible.length === 0 ? (
        <div className="panel empty-panel min-h-[260px]">
          <ClipboardList size={30} />
          <span>
            No{" "}
            {filter === "all" ? "coursework" : typeLabel(filter).toLowerCase()}{" "}
            has been assigned.
          </span>
        </div>
      ) : (
        <div className="grid gap-4 xl:grid-cols-2">
          {visible.map((assignment) => {
            ...
            return (
              <article className="panel p-5" key={assignment.id}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span className="tag neutral">
                      {assignment.work_mode?.toUpperCase() ||
                        typeLabel(assignmentType(assignment))}
                    </span>
                  </div>
                  <span className="text-right text-xs text-[var(--muted)]">
                    <strong className="block text-base text-[var(--ink)]">
                      {record?.score ?? assignment.max_marks}
                    </strong>
                    {record?.score != null
                ...
                <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[var(--line)] pt-4 text-xs text-[var(--muted)]">
                  ...
                  <span
                    className={`status-pill ${status === "submitted" || status === "graded" ? "on-track" : isPastDeadline(assignment.due_date) ? "at-risk" : "needs-attention"}`}
                  >
                    {status === "not_started"
                      ? isPastDeadline(assignment.due_date)
                        ? "Closed"
...
export function CourseworkManager({
  ...
  const workflowType: CourseworkType =
    initialType === "assessment"
      ? "assessment"
      : initialType === "lab"
        ? "lab"
  ...
  const selected =
    curriculumCatalog.find((item) => item.id === selectedId) || initialItem;
  ...
  const workflowAssignments = useMemo(
    () => assignments.filter((item) => assignmentType(item) === workflowType),
    [assignments, workflowType],
  );
  const filteredStudents = useMemo(
    ...
    [studentQuery, students, department, section],
  );
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      isFaculty
        ? loadActivityTemplates(session.token)
        : Promise.resolve({} as Record<string, ActivityTemplate>),
    ])
      .then(([data, loadedTemplates]) => {
        if (!active) return;
        setTemplates(loadedTemplates);
        const template = loadedTemplates[initialItem.id];
        if (template) applyTemplate(initialItem, template);
        setAssignments(data.assignments);
        setSubmissions(data.submissions);
        setSubjects(data.subjects);
        setStudents(data.students);
        setSubjectId(subjectForItem(initialItem, data.subjects));
      })
      .catch((caught) => {
        if (active)
          setError(
            caught instanceof Error
              ? caught.message
              : "Coursework could not be loaded",
          );
      })
      .finally(() => {
  ...
  const chooseItem = (item: CurriculumItem) => {
    setCourse(item.courseCode);
    setTrack(item.track);
    setUnit(item.unit);
    setSelectedId(item.id);
    setTitle(item.title);
    setMarks(String(item.suggestedMarks));
    setSubjectId(subjectForItem(item, subjects));
    setEditingId("");
    setNotice("");
  ...
  const changeCatalogFilter = (
    ...
    setUnit(nextUnit);
    const nextItem =
      curriculumCatalog.find(
        (item) =>
          item.courseCode === nextCourse &&
          item.track === nextTrack &&
          item.unit === nextUnit,
      ) ||
      curriculumCatalog.find(
        (item) => item.courseCode === nextCourse && item.track === nextTrack,
      );
    if (nextItem) chooseItem(nextItem);
  };
  ...
  const publish = async (event: React.FormEvent) => {
    ...
    if (
      ...
      setError(
        "Complete every MCQ prompt and all four options before publishing.",
      );
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        subjectId,
        dueDate,
        maxMarks: effectiveMarks,
        description,
        starterCode: workMode !== "mcq" ? starterCode : "",
        testCases:
          workMode !== "mcq"
            ? [{ input: sampleInput, output: expectedOutput, hidden: false }]
            : [],
        assignedUserIds,
        assigned,
        submitted: 0,
        ...
        curriculumItemId: selected.id,
        courseCode: selected.courseCode,
        unitNumber: selected.unit,
        durationMinutes: taskType === "assessment" ? Number(duration) : 0,
        workMode,
        ...
        questions: workMode === "mcq" ? questions : [],
      };
      ...
      const linkedSubject =
        subjects.find((subject) => subject.id === assignment.subject_id) ||
        null;
      setAssignments((current) => [
        { ...assignment, subjects: linkedSubject },
        ...current.filter((item) => item.id !== assignment.id),
      ]);
      setEditingId("");
      setNotice(
        `Published ${typeLabel(taskType).toLowerCase()} work to ${assigned} student${assigned === 1 ? "" : "s"}.`,
      );
      if (isNext && !editingId) {
        const currentIndex = availableCatalog.findIndex(
          (i) => i.id === selected.id,
        );
  ...
  const removeAssignment = async (assignment: AssignmentRecord) => {
  ...
  const editAssignment = (assignment: AssignmentRecord) => {
    const item = curriculumCatalog.find(
      (entry) => entry.id === assignment.curriculum_item_id,
    );
    if (!item) return;
    chooseItem(item);
    ...
    setExpectedOutput(assignment.test_cases?.[0]?.output || "");
    setSampleInput(assignment.test_cases?.[0]?.input || "");
    setHints((assignment.hints || []).join("\n"));
    setEnvironment(assignment.execution_environment || "runner");
    setWorkMode(assignment.work_mode);
    setQuestions(assignment.questions);
    setMarks(String(assignment.max_marks));
    setDueDate(assignment.due_date);
    setDuration(String(assignment.duration_minutes));
    setAudience("selected");
    setSelectedStudentIds(assignment.assigned_user_ids);
    window.scrollTo({ top: 0, behavior: "smooth" });
  };
        ...
        onSaved={(record) => {
          setSubmissions((current) => {
            const index = current.findIndex(
              (item) =>
                item.id === record.id ||
                item.assignment_id === record.assignment_id,
            );
    ...
    <div className="space-y-7">
      <section className="grid gap-3 sm:grid-cols-3">
        <div className="metric-panel">
          <p>Curriculum items</p>
          <strong>{curriculumCatalog.length}</strong>
          <span>Organized by course, track, and unit</span>
          <span className="metric-icon cyan">
            <BookOpenText size={18} />
          </span>
        </div>
        <div className="metric-panel">
          <p>Published {typeLabel(taskType).toLowerCase()}</p>
          <strong>{workflowAssignments.length}</strong>
          <span>
            {workflowAssignments.reduce((sum, item) => sum + item.pending, 0)}{" "}
            pending submissions
          </span>
        ...
        <div className="metric-panel">
          ...
          <span>One action assigns work to the full class</span>
      ...
      <form className="panel resource-form overflow-hidden" onSubmit={publish}>
        ...
        <div className="grid lg:grid-cols-2">
          <section className="border-b border-[var(--line)] p-5 sm:p-6 lg:border-r">
            <div className="mb-5 flex items-center gap-3">
              <span className="grid size-9 place-items-center rounded-md bg-cyan-500/10 text-cyan-600">
                ...
                <div className="segmented-control mt-2 w-full">
                  {(["JAVA", "DBMS"] as CourseCode[]).map((value) => (
                    <button
                      ...
                      key={value}
                      onClick={() => changeCatalogFilter(value, track, unit)}
                      type="button"
                    >
                      {value}
                    </button>
              ...
              <div className="border-l-2 border-cyan-500 bg-[var(--surface-2)] px-4 py-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <strong className="block text-sm">{selected.title}</strong>
          ...
          <section className="border-b border-[var(--line)] p-5 sm:p-6">
            <div className="mb-5 flex items-center gap-3">
              <span className="grid size-9 place-items-center rounded-md bg-amber-500/10 text-amber-600">
            ...
            <fieldset>
              ...
              {taskType === "practice" || taskType === "assessment" ? (
                <div className="segmented-control mt-2 w-full">
                  <button
                    className={"flex-1 " + (workMode === "mcq" ? "active" : "")}
                    onClick={() => changeMode("mcq")}
                    type="button"
                  >
                    <ListChecks size={14} className="mr-1 inline" />
                    {taskType === "assessment" ? "Proctored MCQ" : "MCQs"}
                  </button>
                  <button
                    className={"flex-1 " + (workMode === "ide" ? "active" : "")}
                    onClick={() => changeMode("ide")}
                    type="button"
                  >
                ...
                <div className="mt-2 rounded-md border border-[var(--line)] bg-[var(--surface-2)] px-4 py-3 text-sm font-semibold">
                  <Code2 className="mr-2 inline text-cyan-600" size={15} />
                  {workMode === "response"
                    ? "Design / written experiment"
                    : "IDE experiment"}
                </div>
              )}
            </fieldset>
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              {workMode === "mcq" ? (
                <div>
                  ...
                  <strong className="mt-2 block rounded-md border border-[var(--line)] bg-[var(--surface-2)] px-3 py-2.5 text-sm">
                    {questionMarks} from questions
                  </strong>
                </div>
              ) : (
                <label>
            ...
            <p className="mt-5 flex items-center gap-2 text-xs text-[var(--muted)]">
              <CalendarDays size={15} />
              Due {dueLabel(dueDate)} for {effectiveMarks} marks
            </p>
          </section>
        </div>
        ...
        <section className="border-b border-[var(--line)] p-5 sm:p-6">
              ...
              <div>
                ...
                <h3 className="text-sm font-semibold">
                  {workMode === "mcq"
                    ? "Build questions"
                    : taskType === "lab"
                      ? "Configure experiment workspace"
                      : "Configure IDE question"}
                </h3>
              </div>
            </div>
          </div>
          <label className="mb-4 block">
            ...
            <textarea
              className="mt-2 min-h-40 w-full rounded-md border border-[var(--line)] bg-[var(--surface-2)] p-3 text-sm leading-6"
              required
              value={description}
          ...
          {workMode !== "mcq" && (
            <div className="mb-4 grid gap-4 sm:grid-cols-2">
              <label>
                Sample input
                <textarea
                  className="mt-2 w-full rounded-md border border-[var(--line)] bg-[var(--surface-2)] p-3 font-mono text-xs"
                  rows={3}
                  value={sampleInput}
          ...
          {taskType !== "assessment" && (
            <label className="mb-4 block">
              ...
              <textarea
                className="mt-2 w-full rounded-md border border-[var(--line)] bg-[var(--surface-2)] p-3 text-sm"
                rows={3}
                value={hints}
          ...
          {workMode === "mcq" ? (
            <div className="space-y-4">
              {questions.map((question, questionIndex) => (
                <article
                  className="rounded-md border border-[var(--line)] bg-[var(--surface-2)] p-4"
                  key={question.id}
                >
                  <div className="flex items-center gap-3">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <label>
                Starter code
                <textarea
                  className="mt-2 min-h-40 w-full resize-y rounded-md border border-[var(--line)] bg-[var(--surface-2)] p-3 font-mono text-xs leading-5 text-[var(--ink)] outline-none focus:border-cyan-500"
                  value={starterCode}
              ...
              <label>
                ...
                <textarea
                  className="mt-2 min-h-40 w-full resize-y rounded-md border border-[var(--line)] bg-[var(--surface-2)] p-3 font-mono text-xs leading-5 text-[var(--ink)] outline-none focus:border-cyan-500"
                  placeholder="Enter the expected program or query output"
                  required
        ...
        <section className="border-b border-[var(--line)] p-5 sm:p-6">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <span className="grid size-9 place-items-center rounded-md bg-emerald-500/10 text-emerald-600">
                <Users size={17} />
              </span>
              <div>
            ...
            {audience === "selected" && (
              ...
              >
                ...
                <div className="max-h-52 overflow-y-auto border-t border-[var(--line)]">
                  ...
                  {filteredStudents.length > renderedStudents.length && (
                    <p className="p-3 text-center text-[10px] text-[var(--muted)]">
                      Showing 100 of {filteredStudents.length}. Search or use
                      Select filtered.
                    </p>
                  )}
                </div>
        ...
        <details className="border-b border-[var(--line)]">
          ...
          <div className="grid gap-4 border-t border-[var(--line)] bg-[var(--surface-2)] p-5 sm:p-6">
            ...
            <label>
              Instructions
              <textarea
                className="min-h-28 w-full resize-y rounded-md border border-[var(--line)] bg-[var(--surface)] p-3 text-xs leading-5 text-[var(--ink)] outline-none focus:border-cyan-500"
                required
                value={description}
                onChange={(event) => setDescription(event.target.value)}
              />
              ...
              >
                {subjects.map((subject) => (
                  <option value={subject.id} key={subject.id}>
                    {courseForSubject(subject)} / {subject.section}
                  </option>
                ))}
              </select>
            </label>
          ...
          <div className="min-w-0">
            {notice ? (
              ...
              <p className="text-xs text-[var(--muted)]">
                ...
                {selected.title} to {recipientCount} student
                {recipientCount === 1 ? "" : "s"}
              </p>
            )}
            <p className="mt-1 flex items-center gap-2 text-[10px] text-[var(--muted)]">
              <FileCode2 size={13} />
      ...
      <section className="panel overflow-hidden">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--line)] p-5">
          <div>
            ...
            <h2 className="mt-1 text-base font-semibold">Assignment queue</h2>
          </div>
          <span className="text-xs text-[var(--muted)]">
            {workflowAssignments.length} total
          </span>
        </header>
              ...
              {workflowAssignments.map((assignment) => (
                <tr key={assignment.id}>
                  <td>
                    ...
                    <small>{assignment.max_marks} marks</small>
                  </td>
                  <td>
                    <span className="tag neutral">
                      {assignment.work_mode?.toUpperCase() ||
                  ...
                  <td>
                    <span
                      className={
                        "tag " +
                        (assignmentCourse(assignment) === "JAVA"
                          ? "cyan"
                          : "amber")
                      }
                    >
                      {assignmentCourse(assignment)} / U
                      {assignment.unit_number || 1}
                    </span>
                  </td>
                  <td>{dueLabel(assignment.due_date)}</td>
              ...
              {workflowAssignments.length === 0 && (
                <tr>
                  <td colSpan={7} className="text-center">
                    No {typeLabel(taskType).toLowerCase()} work has been
                    published.
                  </td>
                </tr>
              )}
