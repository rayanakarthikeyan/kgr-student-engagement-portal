...
export function LabWorkspace({ session, onEvent, theme }: LabWorkspaceProps) {
  ...
  const challenge = useMemo(
    () =>
      labChallenges.find((item) => item.id === challengeId) || labChallenges[0],
    [challengeId],
  );
  ...
  >(() =>
    Object.fromEntries(
      labChallenges.map((item) => [item.id, item.starterCode]),
    ),
  );
  ...
  const [bottomTab, setBottomTab] = useState<"output" | "timeline" | "chat">(
    "output",
  );
  ...
  const handleSendChat = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setChatHistory((curr) => [
        ...
        {
          role: "model",
          content: "Sorry, I am currently unavailable or you are offline.",
        },
      ]);
    } finally {
      setIsChatting(false);
  ...
  return (
    <div className="lab-shell mx-auto max-w-[1540px] overflow-hidden rounded-lg border border-[var(--line)] bg-[var(--surface)] text-[var(--ink)] shadow-xl shadow-slate-900/5">
      <header className="flex min-h-14 flex-wrap items-center gap-3 border-b border-[var(--line)] bg-[var(--surface)] px-3 sm:px-4">
          ...
          >
            {labChallenges.map((item) => (
              <option value={item.id} key={item.id}>
                {item.language.toUpperCase()} · {item.title}
              </option>
            ))}
          </select>
          <ChevronDown
        ...
        <span className="ml-auto hidden items-center gap-2 text-xs text-[var(--muted)] md:flex">
          <Clock3 size={14} />
          Autosaved locally
        </span>
        <button
      ...
      <div className="grid min-h-[720px] lg:grid-cols-[minmax(310px,38%)_minmax(0,62%)]">
        <aside className="overflow-y-auto border-b border-[var(--line)] bg-[var(--surface-2)] lg:border-b-0 lg:border-r">
          <div className="border-b border-[var(--line)] px-5 py-4">
            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.13em] text-[var(--accent)]">
              <FileCode2 size={14} />
              Problem statement
            </div>
            ...
            <p className="mt-1 text-xs text-[var(--muted)]">
              {challenge.topic} · {challenge.language.toUpperCase()}
            </p>
          </div>
          <div className="space-y-6 p-5 text-sm leading-6 text-[var(--muted)]">
            <section>
              <h3 className="problem-heading">Task</h3>
              <p className="mt-2">{challenge.statement}</p>
            </section>
            <section className="lab-theory-bridge rounded-md border p-4">
              <h3 className="flex items-center gap-2 text-xs font-semibold text-[var(--accent)]">
                <Lightbulb size={15} />
                Theory bridge
              </h3>
            ...
            <section>
              <h3 className="problem-heading">Constraints</h3>
            ...
            <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
              <div>
                <h3 className="problem-heading">Sample input</h3>
                <pre className="code-sample">{challenge.sampleInput}</pre>
              </div>
              <div>
                <h3 className="problem-heading">Expected output</h3>
                <pre className="code-sample">{challenge.expectedOutput}</pre>
              </div>
            </section>
            <details className="rounded-md border border-[var(--line)] bg-[var(--surface)] p-4">
              <summary className="cursor-pointer text-xs font-semibold text-amber-500">
                Reveal conceptual hint
              </summary>
              <p className="mt-3 text-xs leading-5">{challenge.hint}</p>
            </details>
          </div>
        </aside>
        ...
        <section className="grid min-h-[720px] min-w-0 grid-rows-[minmax(390px,58%)_minmax(260px,42%)]">
          <div className="min-h-0 border-b border-[var(--line)]">
            <div className="flex h-10 items-center border-b border-[var(--line)] bg-[var(--surface)] px-3">
              <span className="flex h-full items-center gap-2 border-b-2 border-cyan-500 px-2 text-xs text-[var(--ink)]">
                ...
                Main.{challenge.language === "java" ? "java" : "sql"}
              </span>
              <button
                className="ml-auto p-2 text-[var(--muted)] hover:text-[var(--accent)]"
                onClick={() =>
          ...
          <div className="min-h-0 bg-[var(--surface)]">
            <div className="flex h-10 items-center border-b border-[var(--line)] px-3">
              ...
              >
                <History size={14} />
                Attempt timeline <span>{telemetry.timeline.length}</span>
              </button>
              <button
                className={`lab-tab ${bottomTab === "chat" ? "active" : ""}`}
              ...
              {output.status !== "idle" && (
                <span
                  className={`ml-auto flex items-center gap-1.5 text-[11px] ${output.status === "passed" ? "text-emerald-600" : "text-rose-600"}`}
                >
            ...
            {bottomTab === "output" ? (
              <div className="grid h-[calc(100%_-_40px)] min-h-0 md:grid-cols-2">
                <div className="border-b border-[var(--line)] p-4 md:border-b-0 md:border-r">
                  <p className="terminal-label">Expected output</p>
                  <pre className="terminal-output text-[var(--ink)]">
                    {challenge.expectedOutput}
                  </pre>
                </div>
                <div className="p-4">
                  <p className="terminal-label">Actual output</p>
                  {running ? (
                    ...
                    <pre
                      className={`terminal-output ${output.status === "passed" ? "text-emerald-500" : output.status === "idle" ? "text-[var(--muted)]" : "text-rose-500"}`}
                    >
              ...
              <div className="max-h-[240px] overflow-y-auto p-4">
                {telemetry.timeline.length === 0 ? (
                  <div className="grid h-36 place-items-center text-center text-xs text-[var(--muted)]">
                    <span>
                      <History size={24} className="mx-auto mb-2" />
                      Run code or paste into the editor to begin the attempt
                      timeline.
                    </span>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {telemetry.timeline.toReversed().map((item) => (
                      <div
                        className="flex gap-3 rounded-md border border-[var(--line)] bg-[var(--surface-2)] p-3"
                        key={item.id}
                      >
                        <span
                          className={`mt-1 size-2 shrink-0 rounded-full ${item.severity === "success" ? "bg-emerald-500" : item.severity === "error" ? "bg-rose-500" : item.severity === "warning" ? "bg-amber-500" : "bg-slate-400"}`}
                        />
              ...
              <div className="flex h-[calc(100%_-_40px)] flex-col">
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  {chatHistory.length === 0 ? (
                    <div className="flex h-full flex-col items-center justify-center text-center text-[var(--muted)]">
                      ...
                      />
                      <p className="text-sm">
                        I'm your AI tutor. Ask me about errors, concepts, or
                    ...
                    chatHistory.map((msg, i) => (
                      ...
                      >
                        <div
                          className={`max-w-[85%] rounded-lg px-4 py-2.5 text-sm ${msg.role === "user" ? "bg-cyan-600 text-white" : "bg-[var(--surface-2)] border border-[var(--line)] text-[var(--ink)]"}`}
                        >
                          {msg.content}
                        </div>
                      </div>
                  ...
                  {isChatting && (
                    <div className="flex justify-start">
                      <div className="max-w-[85%] rounded-lg bg-[var(--surface-2)] border border-[var(--line)] px-4 py-2.5 text-sm text-[var(--muted)] flex items-center gap-2">
                ...
                >
                  <input
                    ...
                    className="h-9 flex-1 rounded-md border border-[var(--line)] bg-[var(--surface-2)] px-3 text-sm text-[var(--ink)] outline-none focus:border-cyan-500"
                  />
                  <button
                    type="submit"
                    disabled={isChatting || !chatInput.trim()}
                    className="grid h-9 w-9 place-items-center rounded-md bg-cyan-600 text-white disabled:opacity-50"
                  >
                    <ArrowUp size={16} />
                  </button>
                </form>
