...
function documentPreviewUrl(url: string) {
  if (/drive\.google\.com\/file\/d\//i.test(url))
    return url.replace(/\/(view|edit)(\?.*)?$/i, "/preview");
  return url;
}
...
function dueLabel(value: string) {
  ...
  return Number.isNaN(date.getTime())
    ? value
    : `Due ${date.toLocaleDateString(undefined, { day: "numeric", month: "short", year: "numeric" })}`;
}
...
function PdfPlayer({
  ...
  const { containerRef, activeSeconds } = usePdfTracker({
    userId: user.id,
    courseId: resource.courseId,
    resourceId: resource.id,
    onEvent,
  ...
  return (
    <div ref={containerRef}>
      <div className="aspect-[16/10] overflow-hidden rounded-lg border border-[var(--line)] bg-[var(--surface-2)]">
        <iframe
          className="h-full w-full bg-white"
          src={documentPreviewUrl(resource.externalUrl)}
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-xs text-[var(--muted)]">
        <span className="flex items-center gap-2">
          <Clock3 size={14} />
          {activeSeconds}s focused reading tracked while this document is
          visible
        </span>
        <a
...
export function ResourceViewer({
  ...
  const filtered = useMemo(
    () =>
      resources.filter(
        (item) =>
          activeCourseFilter === "all" || item.courseId === activeCourseFilter,
      ),
    [activeCourseFilter, resources],
  );
  const selected =
  ...
  if (!selected)
    return (
      <div className="panel empty-panel min-h-[280px]">
        <BookOpen size={28} />
        <span>No theory resources have been assigned to you.</span>
      </div>
    );
    ...
    <div className="mx-auto grid max-w-[1440px] gap-6 xl:grid-cols-[330px_minmax(0,1fr)]">
      <aside className="panel overflow-hidden">
        <div className="border-b border-[var(--line)] p-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-semibold">Assigned theory</h2>
              <p className="mt-1 text-xs text-[var(--muted)]">
                {filtered.length} videos and readings
              </p>
            </div>
            <ListVideo size={19} className="text-cyan-500" />
          </div>
          <div className="segmented-control mt-4 w-full">
            {[
              ...
              <button
                ...
                key={id}
                onClick={() =>
                  setActiveCourseFilter(id as typeof activeCourseFilter)
                }
                type="button"
              >
                {label}
        ...
        <div className="max-h-[calc(100vh-250px)] space-y-1 overflow-y-auto p-2">
          {filtered.map((resource) => (
            <button
              className={`resource-row ${selected.id === resource.id ? "active" : ""}`}
              key={resource.id}
              onClick={() => setSelectedId(resource.id)}
              type="button"
            >
              <span
                className={`grid size-9 shrink-0 place-items-center rounded-md ${resource.type === "youtube" ? "bg-cyan-500/10 text-cyan-500" : "bg-amber-500/10 text-amber-500"}`}
              >
              ...
              <span className="min-w-0 flex-1 text-left">
                ...
                <small>
                  {resource.courseCode} / Unit {resource.unitNumber} ·{" "}
      ...
      <section className="min-w-0">
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="flex flex-wrap gap-2">
              <span
                className={`tag ${selected.type === "youtube" ? "cyan" : "amber"}`}
              >
                {selected.type === "youtube" ? "Video lesson" : "Reading"}
              </span>
              <span className="tag neutral">
                {selected.courseCode} / Unit {selected.unitNumber}
              </span>
            </div>
            <h2 className="mt-3 text-2xl font-semibold">{selected.title}</h2>
            <p className="mt-1 text-sm text-[var(--muted)]">{selected.topic}</p>
          </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          ...
          <div className="panel p-4">
            <span className="text-xs text-[var(--muted)]">Expected study</span>
            <strong className="mt-2 block text-lg">
              {selected.durationMinutes} minutes
            </strong>
          </div>
          <div className="panel p-4">
            <span className="text-xs text-[var(--muted)]">Tracking</span>
