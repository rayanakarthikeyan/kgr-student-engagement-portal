...
import { Bot, ChevronRight, Plus, ScanSearch, Search, Sparkles } from "lucide-react";
  ...
  return (
    <section className="reference-workspace">
      <aside className="reference-assignment-rail">
        <div className="rail-header"><div><p className="eyebrow">Course work</p><h2>{canManage ? "Assignments" : "My Assignments"}</h2></div>{canManage&&<button className="button compact" type="button" onClick={()=>setShowCreate((value)=>!value)}><Plus size={15}/>New</button>}</div>
        ...
        {showCreate&&<form className="quick-create" onSubmit={createAssignment}><input required placeholder="Assignment title" value={draft.title} onChange={(event)=>setDraft((value)=>({...value,title:event.target.value}))}/><select value={draft.subjectId} onChange={(event)=>setDraft((value)=>({...value,subjectId:event.target.value}))}>{subjects.map((subject)=><option value={subject.id} key={subject.id}>{subject.name}</option>)}</select><div><input required type="date" value={draft.dueDate} onChange={(event)=>setDraft((value)=>({...value,dueDate:event.target.value}))}/><input required min="1" type="number" value={draft.assigned} onChange={(event)=>setDraft((value)=>({...value,assigned:event.target.value}))}/></div><button className="button" type="submit">Create</button></form>}
        <div className="reference-assignment-list">
          {filteredAssignments.map((assignment)=>{const progress=assignment.assigned?Math.round((assignment.submitted/assignment.assigned)*100):0;return <button className={`reference-assignment-card ${selected?.id===assignment.id?"selected":""}`} type="button" key={assignment.id} onClick={()=>setSelectedId(assignment.id)}><div className="assignment-title-row"><h3>{assignment.title}</h3><ChevronRight size={16}/></div><div className="assignment-meta"><span>{assignment.subjects?.name??"Student group"}</span><span>Due {assignment.due_date}</span></div><div className="reference-mini-stats"><span><strong>{assignment.assigned}</strong>Assigned</span><span className="blue"><strong>{assignment.submitted}</strong>Submitted</span><span className="amber"><strong>{assignment.pending}</strong>Pending</span><span className="green"><strong>{assignment.reviewed}</strong>Graded</span></div><div className="progress strong"><span style={{width:`${progress}%`}}/></div></button>;})}
